...
export default function Preview(){
 ...
 const frame=getFrameById(session?.selectedFrameId); const needed=frame.requiredPhotos; const selected=session?.selectedPhotoIndices??[]; const chosen=selected.length?selected.map(i=>session?.capturedPhotos[i]).filter((x):x is string=>Boolean(x)):(session?.capturedPhotos??[]).slice(-needed);
