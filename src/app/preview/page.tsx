...
export default function Preview(){
 ...
 return <KioskStage><h1 className="preview-heading">PREVIEW FRAME</h1><PreviewComposer frame={frame} photoUrls={chosenDisplayUrls}/><PhotoResultStrip photos={allDisplayUrls} selectedIndices={selected} onTogglePhoto={toggle}/><StickerPicker selectedStickerId={session?.selectedStickerId} onSelectSticker={selectSticker}/><KioskButton className="preview-next" onClick={next} disabled={!isReady || saving}>{saving?"PROCESSING...":"NEXT"}</KioskButton>{!isReady&&<p className="kiosk-message" style={{ color: "#ffaa00", top: "82%" }}>Butuh {needed} foto untuk frame ini. Kamu baru punya {chosen.length} foto.</p>}{error&&<p className="kiosk-message">{error}</p>}</KioskStage>;
}
